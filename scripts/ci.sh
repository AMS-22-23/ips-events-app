...
runDriver () {
    cd $1
    if [ -f "lib/main/main_int.dart" ]; then
...
runIos() {
    cd $1;
    if [ -f "lib/main/main_dev.dart" ]; then
        flutter build ios -t ./lib/main/main_dev.dart
    fi
    cd - > /dev/null
}
...
runClean() {
    ...
    if [ -f "pubspec.yaml" ]; then
        ...
        rm -rf ios/Pods ios/.symlinks ios/Flutter/Flutter.framework ios/Flutter/Flutter.podspec ios/Podfile ios/Podfile.lock
